...
from network.misc import panostretch
...
class PanoCorBonDataset(data.Dataset):
    ...
    def __init__(self, root_dir,
        ...
        self.edge_dir = os.path.join(root_dir, 'EM_gt')
    ...
    def __getitem__(self, idx):
        ...
        H, W = img.shape[:2]
    #Own code: read edge map
        edge_path = os.path.join(self.edge_dir,
                                self.img_fnames[idx])
        #edge = np.array(Image.open(edge_path), np.float32)
...
#        bon = cor_2_1d(cor, H, W) #Central projection boundaries
        bon = bon_nc_edge(edge, H, W) #Non-central projection boundaries
    ...
    for i in range(W):
        for j in range(int(H/2)):
            if (edge[j,i] >= 250).all():
                bon[0,i] = j
                break
        for j in np.arange(H-1,int(H/2),-1):
            if (edge[j,i] >= 250).all():
                bon[1,i] = j
                break
    bon = ((bon + 0.5) / H - 0.5) * np.pi
...
#        print(y_bon)
    ...
    up_bon = y_bon[0]
    down_bon = y_bon[1]
    for k in range(y_bon.shape[1]):
        up_bon[k] = y_bon[0,k] if y_bon[0,k]>=0 else 0
        down_bon[k] = y_bon[1,k] if y_bon[1,k]<512 else 511
    img_bon[up_bon, np.arange(len(up_bon)), 1] = 255
    img_bon[down_bon, np.arange(len(down_bon)), 1] = 255
