...
from network.model import HorizonNet, ENCODER_RESNET, ENCODER_DENSENET
from network.dataset import PanoCorBonDataset
from network.misc.utils import group_weight, adjust_learning_rate, save_model, load_trained_model
from network.inference import inference
from network.eval_general import test_general
    ...
    for ith_epoch in trange(1, args.epochs + 1, desc='Epoch', unit='ep'):
        ...
        if ith_epoch % args.save_every == 0:
            save_model(net,
                       ...
                       args)
        # Periodically reduce learning rate by half
        if ith_epoch % 10 == 0:
            args.running_lr *= 0.5
