...
from network.misc.post_proc import np_coor2xy, np_coorx2u, np_coory2v
from network.misc.panostretch import pano_connect_points
from network.eval_general import layout_2_depth
...
def main(args):
    ...
    if not args.ignore_wireframe:
        ...
        wf_points = [[y, x, ceil_z] for x, y in floor_xy] +\
                    [[y, x, floor_z] for x, y in floor_xy]
    ...
    pcd_name = args.layout
    open3d.io.write_point_cloud(pcd_name[:-4]+'pcd',pcd)

    np.save(pcd_name[:-4]+'npy',wf_points)
...
if __name__ == '__main__':
    ...
    path_img = 'Test_sets/comparison/central/img/'
    path_layout = 'Test_sets/comparison/central/results/FT_central/json/'

    img_list = os.listdir(path_img)
    layout_list = os.listdir(path_layout)
    img_list.sort()
    layout_list.sort()
    for i in trange(len(img_list)):
        img_default = img_list[i]
        layout_default = layout_list[i]
        parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
        parser.add_argument('--img', default=path_img+img_default,
                            help='Image texture in equirectangular format')
        parser.add_argument('--layout', default=path_layout+layout_default,
                            help='Txt file containing layout corners (cor_id)')
        parser.add_argument('--scale', default=1, type=float,
                            help='Scale texture for visualization')
        parser.add_argument('--ignore_floor', action='store_true',
